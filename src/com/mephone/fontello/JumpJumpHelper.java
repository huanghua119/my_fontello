...
public class JumpJumpHelper {
    ...
    private int[] getHalmaAndBoardXYValue(File currentImage) throws IOException {
        ...
        if (halmaXSum != 0 && halmaXCount != 0) {
            ...
            for (int y = gameScoreBottomY; y < height; y++) {
                ...
                for (int x = 0; x < width; x++) {
                    ...
                    if ((Math.abs(pixelR - lastPixelR)
                            + Math.abs(pixelG - lastPixelG) + Math.abs(pixelB
                            - lastPixelB)) > 10) {
                        boardXSum += x;
                        boardXCount++;
                    }
            ...
            boardY = (int) (halmaY - Math.abs(boardX - halmaX)
    ...
    public static void main(String[] args) {
        try {
            ...
            for (;;) {
                ...
                if (result == null) {
                    System.out
                            .println("The result of method getHalmaAndBoardXYValue is null!");
                    continue;
                }
                int halmaX = result[0];
                int halmaY = result[1];
                ...
                // 计算跳跃的距离
                double jumpDistance = jumpjumpHelper.computeJumpDistance(
                        halmaX, halmaY, boardX, boardY);
