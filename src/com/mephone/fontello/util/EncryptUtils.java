...
public enum EncryptUtils {
    ...
    public static final int ENCRYPT_TYPE_TEA = 1;

    public String Encrypt(String data, String key, int type) {
        String res = null;
        switch (type) {
        case ENCRYPT_TYPE_TEA:
            res = ToHexString(TEAEncrypt(
    ...
    public String Decrypt(String data, String key, int type) {
        String res = null;
        switch (type) {
        case ENCRYPT_TYPE_TEA:
            ...
            byte[] code = ToByteArray(TEADecrypt(
                    ...
                    ToLongArray(PadRight(key, MIN_LENGTH).getBytes(
                            Charset.forName("UTF8")))));
            res = new String(code, Charset.forName("UTF8"));
        }

        return res;
    }

    private long[] TEAEncrypt(long[] data, long[] key) {
        int n = data.length;
        if (n < 1) {
            return data;
        }
    ...
    private long[] TEADecrypt(long[] data, long[] key) {
        int n = data.length;
        if (n < 1) {
            return data;
        }
    ...
    private String ToHexString(long[] data) {
    ...
    private String PadRight(String source, int length) {
